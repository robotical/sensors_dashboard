...
import Graph from "../GraphLogisticRegression";
import GraphControls from "../GraphControls";
import styles from "../GraphArea/styles.module.css";
import { CSVLink } from "react-csv";

interface GraphAreaLogisticRegressionProps {
  graphId: string;
...
export default function GraphAreaLogisticRegression({ graphId, removeGraph }: GraphAreaLogisticRegressionProps) {
  ...
  const isOnAir = useRef<boolean>(false);
  ...
  const onAddonChange = (evt: ListenerOptionsType) => {
    ...
    const isOnAirCalc = evt.value! > 4;
    isOnAir.current = isOnAirCalc;
    graphData.current[traceId].x.push(evt.value!);
    graphData.current[traceId].y.push(+isOnAirCalc);
  ...
  return (
    <div className={styles.graphArea} style={{backgroundColor: isOnAir.current ? "green" : "red"}}>
